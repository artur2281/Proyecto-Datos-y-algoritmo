...
def iniciar_sesion():
    ...
    resultado_label.config(text="Iniciando sesión...")  # Muestra un mensaje de carga

    # Simula un tiempo de carga (3 segundos en este caso)
    time.sleep(3)

    if usuario_ingresado in usuarios and contrasena_ingresada == usuarios[usuario_ingresado]:
        resultado_label.config(text="Inicio de sesión exitoso para {}".format(usuario_ingresado))
    else:
        resultado_label.config(text="Inicio de sesión fallido. Verifica tus credenciales.")
